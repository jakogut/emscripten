...
def main():
  ...
  for what in tasks:
    ...
    if what == 'compiler-rt':
      ...
      build_port('regal', libname('libregal.bc'), ['-s', 'USE_REGAL=1'])
      ...
      ''', [shared.static_library_name('libc-sockets')])
      ...
      ''', [shared.static_library_name('libc-sockets-proxy')], ['-s', 'PROXY_POSIX_SOCKETS=1', '-s', 'USE_PTHREADS=1', '-s', 'PROXY_TO_PTHREAD=1'])
    else:

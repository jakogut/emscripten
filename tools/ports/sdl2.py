...
def get(ports, settings, shared):
  ...
  def create():
    ...
    thread_srcs = [
        '{backend}/SDL_syscond.c',
        'generic/SDL_sysmutex.c',
        '{backend}/SDL_syssem.c',
        '{backend}/SDL_systhread.c',
        '{backend}/SDL_systls.c'
    ]

    thread_backend = 'generic' if not settings.USE_PTHREADS else 'pthread'
    srcs += ['thread/%s' % s.format(backend=thread_backend) for s in thread_srcs]
